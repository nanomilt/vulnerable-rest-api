...
router.get('/:name', auth, async(req,res)=>{
    ...
    if(regex.test(req.params.name)){
        ...
        return res.render('user', _.pick(user, ['name', 'email', 'role', 'username', 'website', '_id', 'credit'])); // Use res.render() to render safely escaped HTML
    }
...
router.put('/:id', [auth, validateObjectId], async(req, res)=>{
    ...
    await needle('get', req.body.url)
        ...
        .catch(function() { /* Intentionally left blank */ })
    ...
    res.render('status', {status: 'Updated', domain}); // Use res.render() to render safely escaped HTML
})
