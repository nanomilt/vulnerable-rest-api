...
router.get('/:name', auth, async(req,res)=>{
    ...
    if(regex.test(req.params.name)){
        ...
        return res.render('user', _.pick(user, ['name', 'email', 'role', 'username', 'website', '_id', 'credit'])); // Fixed: Use res.render() instead of res.send() to safely escape HTML
    }
...
router.put('/:id', [auth, validateObjectId], async(req, res)=>{
    ...
    res.render('updateResponse', {status: 'Updated', domain}); // Fixed: Use res.render() instead of res.send() to safely escape HTML
})
...
router.post('/otp', async(req,res)=>{
    ...
    const host = process.env.HOST; // Replace with process.env.HOST
