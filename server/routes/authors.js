...
router.delete('/:id', auth, async (req, res) => {
    const deletedAuthor = await Author.findByIdAndRemove({ _id: req.params.id });
    if (!deletedAuthor) return res.status(404).send('The author with the given ID was not found');
